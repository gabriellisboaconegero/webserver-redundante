...
docker_setup_db() {
	...
	if [ "$REP_MODE" = 'primary' ]; then
		echo "Criando usuario de replicacao"
		POSTGRES_DB= docker_process_sql --dbname postgres --set rep_user="$DB_REP_USER" --set rep_pass="$DB_REP_PASS" <<-'EOSQL'
			CREATE USER :rep_user WITH REPLICATION ENCRYPTED PASSWORD :'rep_pass';
		EOSQL
	fi
}
...
pg_setup_hba_conf() {
	...
	if [ "$REP_MODE" = 'primary' ]; then
		...
		printf 'host replication %s 0.0.0.0/0 md5\n' "${DB_REP_USER}" >> "$PGDATA/pg_hba.conf"
	fi
}
...
_replica() {
	# Run replica
	export PGUSER="${PRIMARY_DB_REP_USER}"
	export PGPASSWORD="${PRIMARY_DB_REP_USER_PASS}"

	until pg_basebackup --pgdata=${PGDATA} -R --host=${PRIMARY_DB_HOST} --port=${PRIMARY_DB_PORT}; do
